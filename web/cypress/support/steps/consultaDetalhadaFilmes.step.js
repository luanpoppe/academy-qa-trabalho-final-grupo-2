    ...
    cy.createUserAndMovie(movieBody).then((response) => {
      ...
      cy.createCriticUser().then((response) => {
        ...
        cy.reviewMovie(
          idMovie,
          2,
          "são pouco velozes e nada furiosos",
          userCritic.accessToken
        );
      });
      cy.createCriticUser().then((response) => {
      ...
      cy.createUser().then((response) => {
        ...
        cy.login(userComum).then(function (response) {
          ...
          cy.reviewMovie(
            idMovie,
            3,
            "são muito velozes e pouco furiosos",
            userComum.accessToken
          );
        });
      });
      cy.createUser().then((response) => {
        ...
        cy.login(userComum2).then(function (response) {
          ...
          cy.reviewMovie(
            idMovie,
            4,
            "são muito velozes e nada furiosos",
            userComum2.accessToken
          );
        });
      });
    });
...
Then(
  ...
  () => {
    cy.get(paginaMovies.labelAudience)
      .should("be.visible")
      .and("contain", "Avaliação da audiência");
    cy.get(paginaMovies.labelQtdAudience)
      .should("be.visible")
      .and("contain", "2 avaliações");
  }
);
...
Then(
  ...
  () => {
    cy.get(paginaMovies.labelCritic)
      .should("be.visible")
      .and("contain", "Avaliação da crítica");
    cy.get(paginaMovies.labelQtdCritic)
      .should("be.visible")
      .and("contain", "2 avaliações");
  }
);
...
Then("deverá visualizar todas as informações do filme selecionado", () => {
  cy.get(paginaMovies.labelTitle)
    .should("be.visible")
    .and("contain", "Velozes e Furiosos 10");
  cy.get(paginaMovies.labelGenre).should("be.visible").and("contain", "Ação");
  cy.get(paginaMovies.labelDescription)
    .should("be.visible")
    .and("contain", "O fim da estrada esta chegando");
...
Then(
  ...
  () => {
    ...
    cy.get(paginaMovies.singleStarsAudience + " .filled ").should(
      "have.length",
      4
    );
    cy.get(paginaMovies.singleStarsCritic + " .filled ").should(
      "have.length",
      1
    );
    cy.get(paginaMovies.singleStarsCritic + " .filled-half ").should(
      "have.length",
      1
    );
  }
